...
class XArmControl(Thread):
    def __init__(self, ip = "192.168.1.169"):
            ...
            if self.mode == 0:
                ...
                if not self.is_radian:
                    if not ( abs(self.currentPosition['x'] - self.position['x']) > self.xyzThreshold
                        ...
                        self.position.update(pos)
                        if 'roll' in pos:
                            # if self.currentPosition['roll'] < 0: #(self.currentPosition['roll'] < 0 and pos['roll'] > 0) or (self.currentPosition['roll'] > 0 and pos['roll'] < 0):
                            #     if pos['roll'] > 0:
                            # if self.currentPosition['roll'] < 0:
